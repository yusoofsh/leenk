...
| Analytics  | `tabs`, `chart`, `card`, `table`, `select`, `alert`, `skeleton`, `button`                                                   | `leenk_shortlinks` and `leenk_site_events` via the server API                                  | Tabs for Shortlinks, Site events; range select 7/30/90 days (30 default, 90 max); weighted counts labeled "Analytics Engine counts" with a sampling note; legacy rows shown as codes until retention expires; link-outs to Cloudflare Web Analytics and Observability |
